...
print('''Welcome! This is a text colorizer for BBCode and HTML. Enter your text, code preference, and desired color scheme, and this will output BBCode/HTML for you to copy/paste that compiles to your desired aesthetic.
''')
...
def errmsg():
  print('''you probabaly entered a color that hasn't been added yet, or your hex color code is invalid. don't forget to put a hashtag at the beginning of the 6 characters. Start from the beginning, loser!
  quitting...
  ''')
  time.sleep(6)
  quit()
...
class Alternating_Color(Text_Edit):
  def evaluateN(self, colorset):
    n = self.n
    bbcode = ''
    for i in range(len(self.text)):
      bbcode+="[color="+colorset[i%n]+"]"+self.text[i]+"[/color]"
    return bbcode
  def evaluateNhtml(self, colorset):
    n = self.n
    html = ''
    for i in range(len(self.text)):
...
class Two_Color_Shift(Text_Edit): 
#this is used only as an operator for the N_Color class, and contains the code to interpolate from one color to the next over a certain range
...
class N_Color_Shift(Two_Color_Shift):
  def evaluateN(self, colorset):
    n = self.n
...
def Alternating_Prompt(text, n):  
  #each input being changed to hex color code format
  colorset=[]
  for i in range(n): colorset+=[None]
    
  for i in range(n):
    colorset[i] = input('ENTER COLOR ' + str(i+1) + ''' (entering the hexcode as '#0a1b2c' or a color in lower case like "orange" should work):
    ''')
    if colorset[i] in colordict: #shortcutting for colors
      colorset[i] = colordict[colorset[i]]
    elif not (len(colorset[i])==7 and colorset[i][0]=="#"): errmsg()
...
def Alternating_Rainbow_Prompt(text, n):  
  #each input being changed to hex color code format
  ...
  return [bbcode,html,colorlist]
def N_Color_Shift_Prompt(text, n):  
  #each input being changed to hex color code format
  colorset=[]
  for i in range(n): colorset+=[None]
...
def Rainbow_Shift_Prompt(text, n):
...
def main():
  #entering text to be changed
  text = input('''ENTER TEXT TO BE EDITED:
  ''')
  code = input('''Would you like bbcode or html? (type 'bbcode' or 'html')
  ''')
  while code not in ['html', 'bbcode']: code = input('''That's not one of the options! Please type 'html' or 'bbcode'.
    ''')
  ...
  while option not in ["1", "2", "3", "4"]:
    option = input('''That's not one of the options! Please type the number corresponding to the type of stylization you want.
    ''')
  ...
  if option == '1': 
    ...
    while type(n) == str: 
      try: 
        n = int(n)
      except: n = input('''please enter an integer!
      ''')
...
msg = tk.Label(text = ("your " + code + " blurb is below! feel free to copy+paste"), bg = "white")
msg.place(x = 10, y = 10)


#box for copypaste
codebox = tk.Text(width = 65, height = 10)
codebox.place(x = 10, y = 35)
if code == "html": code = html
elif code == "bbcode": code = bbcode
